...
apt-get -y --no-install-recommends install linux-image-armmp abootimg cgpt \
  fake-hwclock u-boot-tools vboot-utils vboot-kernel-utils alsa-utils pulseaudio \
