...
{
	names=($(lsblk -d -n -p -o NAME))
	transports=($(lsblk -d -n -p -o TRAN))
	hotplugs=($(lsblk -d -n -p -o HOTPLUG))
	models=($(lsblk -d -n -p -o MODEL))
	vendors=($(lsblk -d -n -p -o VENDOR))
	sizes=($(lsblk -d -n -p -o SIZE))
	echo "Discovered devices"
	echo "------------------"
	for item in ${!names[*]} ; do
	    printf "Path=%s\tSize=%s\tModel="%s"\tVendor=%s\tTransport=%s\tHotplug=%s\n" "${names[$item]}" "${sizes[$item]}" "${models[$item]}" "${vendors[$item]}" "${transports[$item]}" "${hotplugs[$item]}"
	done
	echo ""
	...
	for item in ${!names[*]}
	do
		if [ "${transports[$item]}" == "usb" ] && [ "${hotplugs[$item]}" == "1" ]; then
			printf "%d: Path=%s\tModel="%s"\tVendor=%s\tTransport=%s\tHotplug=%s\n" "$count" "${names[$item]}" "${models[$item]}" "${vendors[$item]}" "${transports[$item]}" "${hotplugs[$item]}"
...
{
	for partition in `fdisk $1 -l | grep "^/dev/" | cut -d" " -f1`
	do
...
function format_device
{
	...
	cgpt add -i 3 -t data -b 73728 -s `expr $(cgpt show $1 | grep 'Sec GPT table' | awk '{ print \$1 }')  - 73728` -l Root $1
	blockdev --rereadpt $1
	#cgpt show $1
